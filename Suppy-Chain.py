...
# Algorithm for generating a proof-of-work (based on PoW)
...
def make_transaction(supplier_key, receiver_key, item_id):
    ...
    selection = input('\nSelect type of key (M/S) for supplier: ')
    if selection == 'M':
        index = int(input('There are a total of ' +
                          str(len(manufacturers_list)) + ' users. Enter your selection: ')) - 1
        supplier_key = manufacturers_list[index]

    elif selection == 'S':
        index = int(input('There are a total of ' +
                          str(len(other_users_list)) + ' users. Enter your selection: ')) - 1
        supplier_key = other_users_list[index]

    selection = input('\nSelect type of key (M/S) for receiver: ')
    if selection == 'M':
        index = int(input('There are a total of ' +
                          str(len(manufacturers_list)) + ' users. Enter your selection: ')) - 1
...
def mine_block():
    ...
    if(transaction_amount):
        ...
        if(len(transaction_array) != 0):
            new_block = Supply_Block(global_index, date.datetime.now(
            ), transaction_array, supply_blockchain[global_index - 1].hash)
            global_index = global_index + 1
...
while(1):
    ...
    if(choice == 1):
        ...
        print('This is an invalid option, Do you want to view the Blockchain?')
